...
<html lang="en">
	...
	<body>
		<div style="padding: 20px 20px 10px 20px">
			<div style="">
				<div><img src="data:image/png;base64,{{$image}}" alt="logo" width="200px" /></div>
				<div
			...
			<div>
				<table style="width: 100%">
					<tr style="height: 50px">
						<td width="70%">Bill to : {{ $current_order->bill_to }}</td>
						<td width="30%">Phone : {{ $current_order->phone_number }}</td>
					</tr>
					<tr style="height: 50px">
						<td>Address : {{ $current_order->address }}</td>
						<td>Work order :  {{ $current_order->work_order }}</td>
					</tr>
					<tr style="height: 50px">
						<td>City : {{ $current_order->city }}</td>
						<td>Order Taken by : {{ $current_order->order_taken_by }}</td>
					</tr>
					<tr style="height: 50px">
						<td>Email : {{ $current_order->email }}</td>
						<td>REF Quote : {{ $current_order->ref }}</td>
					</tr>
				</table>
			</div>
            ...
            <div style="padding: 10px 0px 0px 0px">
				<table style="width: 100%">
					<tr style="height: 50px">
						<td style="width: 30%">Start Date : {{ $start_date }}</td>
						<td style="width: 30%">Start Time : {{ $current_order->start_time }}</td>
					...
					<tr style="height: 50px">
						...
						<td>End Time : {{ $current_order->end_time }}</td>
            ...
            <div style="padding: 10px 0px 0px 0px">
				...
				<p style="color:rgb(39, 137, 199); padding-bottom:5px">Technician Names: {{ $current_order->tech_name }}</p>
			</div>
            ...
            <div style="padding: 10px 0px 0px 0px;color: rgb(39, 137, 199);">
				...
				<div style="width: 45%;">
					...
					<span>
						<img src="{{ $current_order->signature }}" alt="logo" width="200px">
					</span>
